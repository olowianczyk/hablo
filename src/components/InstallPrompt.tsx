...
export function InstallPrompt() {
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') setShowInstall(false);
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [showInstall, setShowInstall]);
  ...
  return (
    <div onClick={dismiss} style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div
        ...
        style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 22, padding: 30, maxWidth: 400, width: '100%', textAlign: 'center', boxShadow: '0 30px 70px rgba(0,0,0,.35)' }}
      >
        ...
        <div id="install-title" style={{ fontSize: 20, fontWeight: 800, marginTop: 18, color: 'var(--ink)' }}>{t.installTitle}</div>
        ...
        <Tap onClick={install} style={{ marginTop: 20, background: 'var(--accent)', color: '#fff', fontWeight: 800, fontSize: 15, padding: 14, borderRadius: 14, width: '100%' }}>{t.installBtn}</Tap>
        <Tap onClick={dismiss} style={{ marginTop: 10, color: 'var(--muted)', fontWeight: 700, fontSize: 13.5, width: '100%' }}>{t.installLater}</Tap>
      </div>
    </div>
  );
}
