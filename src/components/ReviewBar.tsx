...
export function ReviewBar() {
  ...
  return (
    ...
    >
      ...
      <Tap onClick={() => reviewNext()} style={{ background: 'var(--accent)', color: '#fff', fontWeight: 800, fontSize: 13, padding: '9px 16px', borderRadius: 10 }}>{nextLabel} →</Tap>
      <Tap onClick={exitReview} aria-label="Exit review" style={{ color: 'var(--faint)', fontWeight: 700, fontSize: 13, padding: 4 }}>✕</Tap>
    </div>
  );
}
